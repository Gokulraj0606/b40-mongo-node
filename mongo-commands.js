...
// inclusion projection (its show only name & rating)
db.movies.find({}, { name: 1 })
db.movies.find({}, { name: 1, rating: 1 })

// exculsion projection (its show without summary & trailer)
db.movies.find({}, { summary: 0, trailer: 0 })

// mix - ❌
db.movies.find({}, { name: 1, summary: 0 })

// exception (to remove _id)
...
// desc
db.movies.find({}, { name: 1, rating: 1 }).sort({ rating: -1 })
db.movies.find({}, { name: 1, rating: 1 }).sort({ name: -1 })
...
// aggregate - stage 1 filter-urgent orders
db.orders.aggregate([
    { $match: { status: "urgent" } }
])
// [
...
// update
db.movies.find()
// to add language in all movies
db.movies.updateMany({}, { $set: { language: "english" } })

// update language in one movies
...
// to detele movies fully
db.movies.deleteMany({ id: "105" })